...
import { 
  ...
  Edit2, Save, TrendingUp, DollarSign, Calendar, FileText, ImageIcon, Camera, RotateCcw, BatteryMedium
...
const Inicio = () => {
  ...
  const noticiasFit = [
    ...
    { id: 1, titulo: "Mr. Olympia 2026: Preparação dos atletas brasileiros ganha destaque mundial", img: "https://images.unsplash.com/photo-1517836357463-d25dfeac3438?w=500&auto=format&fit=crop&q=60" },

    { id: 2, titulo: "Pesquisa aponta: Creatina aliada a carboidratos aumenta em 30% a absorção", img: "https://images.unsplash.com/photo-1593095948071-474c5cc2989d?w=500&auto=format&fit=crop&q=60" },

    { id: 3, titulo: "Novas regras na categoria Wellness da IFBB mudam o padrão de julgamento", img: "https://images.unsplash.com/photo-1534438327276-14e5300c3a48?w=500&auto=format&fit=crop&q=60" }
  ...
  return (
    ...
    <div className="flex-1 overflow-y-auto pr-2 space-y-6 custom-scrollbar pb-24 text-white">
      ...
      {/* Termômetro de Recuperação (Interativo) */}
      <div className="bg-[#0A1A10] border border-[#1A4026] rounded-2xl p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-[#D4AF37] text-sm font-semibold flex items-center gap-2"><BatteryMedium size={18}/> Nível de Recuperação</h3>
        </div>
        <p className="text-[#A0B3A6] text-xs mb-3">Como está sua musculatura hoje?</p>
        <div className="flex justify-between gap-2 mb-2">
          {[1, 2, 3, 4, 5].map((nivel) => (
            <button 
              key={nivel}
              onClick={() => setRecuperacao(nivel)}
              className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all active:scale-95 ${recuperacao === nivel ? 'bg-[#D4AF37] text-[#0A1A10]' : 'bg-[#1A3020] text-[#A0B3A6] border border-[#1A4026]'}`}
            >
              {nivel}
            </button>
          ))}
        ...
        {recuperacao > 0 && (
          <div className="bg-[#1A3020] border border-[#D4AF37]/30 p-2 rounded-lg text-center mt-2">
            <p className="text-[11px] text-[#D4AF37]">
              {recuperacao <= 2 ? 'Fadiga alta: Priorize um treino regenerativo ou descanso ativo hoje.' : 
               recuperacao === 3 ? 'Recuperação média: Treino moderado focado em técnica é ideal.' : 
               'Recuperação excelente: Pronto para quebrar recordes e focar em progressão de carga!'}
            </p>
          </div>
        )}
      </div>
      ...
      <div className="mt-8">
        ...
        <div ref={scrollRef} className="flex overflow-x-auto gap-4 custom-scrollbar pb-4 -mr-2 pr-2 scroll-smooth">
  ...
  return (
    ...
    <div className="flex-1 overflow-y-auto pr-2 space-y-6 custom-scrollbar pb-24 text-white">
      ...
      <div className="flex gap-4">
        ...
        <button onClick={() => setWaterConsumed(0)} className="w-14 h-14 bg-[#0A1A10] border border-red-900/50 rounded-2xl flex items-center justify-center text-red-500 active:scale-95 transition-transform" title="Esvaziar Copo"><RotateCcw size={24} /></button>
...
const Perfil = () => {
  ...
  const handleSave = async () => {
    ...
    // Evita erro do Postgres ao tentar enviar string vazia para uma coluna de data
    if (!payload.data_nascimento) payload.data_nascimento = null;

    const { error } = await supabase.from('profiles').update(payload).eq('id', profile.id);

    if (error && error.code !== 'PGRST205' && error.code !== '404' && !error.message?.includes('not find the table')) {

      setStatusMsg('Erro: ' + (error.message || 'ao salvar!'));
  ...
  const handleImageUpload = async (e) => {
    ...
    if (uploadError && uploadError.statusCode !== '404' && !uploadError.message?.includes('Bucket not found')) {
      ...
      setStatusMsg('Erro: ' + (uploadError.message || 'ao enviar foto!'));
    ...
    // Se o bucket não existir, salva localmente para não quebrar a experiência
    const publicUrl = uploadError ? URL.createObjectURL(file) : supabase.storage.from('avatars').getPublicUrl(fileName).data.publicUrl;
    ...
    if (updateError && updateError.code !== 'PGRST205' && updateError.code !== '404' && !updateError.message?.includes('not find the table')) {

      setStatusMsg('Erro: ' + (updateError.message || 'ao salvar no perfil!'));
              ...
              <>
                ...
                {showDesempenho && (
                  ...
                  <div className="bg-[#0A1A10] border border-[#1A4026] rounded-2xl p-4">
                    ...
                    {/* Exibição detalhada das medidas do aluno */}

                    {progressoAluno.length > 0 && (

                      <div className="mt-4 grid grid-cols-4 gap-2 text-center text-xs">

                        <div className="bg-[#1A3020] p-2 rounded-lg border border-[#1A4026]">

                          <span className="block text-[#A0B3A6] text-[9px] uppercase">Peso</span>

                          <span className="text-white font-bold">{progressoAluno[progressoAluno.length - 1].peso}kg</span>

                        </div>

                        <div className="bg-[#1A3020] p-2 rounded-lg border border-[#1A4026]">

                          <span className="block text-[#A0B3A6] text-[9px] uppercase">Braço</span>

                          <span className="text-white font-bold">{progressoAluno[progressoAluno.length - 1].braco || '-'}cm</span>

                        </div>

                        <div className="bg-[#1A3020] p-2 rounded-lg border border-[#1A4026]">

                          <span className="block text-[#A0B3A6] text-[9px] uppercase">Cintura</span>

                          <span className="text-white font-bold">{progressoAluno[progressoAluno.length - 1].cintura || '-'}cm</span>

                        </div>

                        <div className="bg-[#1A3020] p-2 rounded-lg border border-[#1A4026]">

                          <span className="block text-[#A0B3A6] text-[9px] uppercase">Coxa</span>

                          <span className="text-white font-bold">{progressoAluno[progressoAluno.length - 1].coxa || '-'}cm</span>
            ...
            <>
              ...
              <header className="flex justify-between items-center px-6 py-4 pt-[calc(1rem+env(safe-area-inset-top))] relative z-10 flex-shrink-0">
                ...
                <div className="flex items-center gap-2 relative">
                  ...
                  <button 
                    ...
                    onClick={() => {
                      ...
                      if (!showNotifications && profile) {
                        ...
                        supabase.from('notificacoes').select('*').eq('user_id', profile.id).then(({data}) => {

                          if(data) setNotificationsList(data.reverse());
                    ...
                    className="w-10 h-10 rounded-full bg-[#051109] flex items-center justify-center text-[#D4AF37] relative"
                  ...
                  {/* Dropdown de Notificações Ajustado (Não encavalado) */}

                  {showNotifications && (

                    <div className="absolute top-[120%] right-0 w-72 bg-[#0A1A10] border border-[#1A4026] rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.8)] overflow-hidden z-[100]">

                      <div className="p-3 border-b border-[#1A4026] flex justify-between items-center">

                        <h3 className="text-[#D4AF37] font-semibold text-sm">Notificações</h3>
                        ...
                        {notifCount > 0 && (
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              supabase.from('notificacoes').update({ lida: true }).eq('user_id', profile.id).then(() => {

                                setNotifCount(0);

                                setNotificationsList(notificationsList.map(n => ({...n, lida: true})));

                              });

                            }}

                            className="text-xs text-[#A0B3A6] hover:text-[#D4AF37]"
                      ...
                      <div className="max-h-64 overflow-y-auto custom-scrollbar">
                        ...
                        {notificationsList.length > 0 ? (
                          ...
                          notificationsList.map((notif, idx) => (
                            ...
                            <div key={notif.id || idx} className={`p-3 border-b border-[#1A4026]/50 last:border-0 flex gap-3 items-start ${notif.lida ? 'opacity-50' : 'bg-[#1A3020]/40'}`}>

                              <div className="w-8 h-8 rounded-full bg-[#051109] border border-[#D4AF37]/30 flex items-center justify-center shrink-0 mt-0.5">
                              ...
                              <div>
                                ...
                                <p className="text-[#D4AF37] text-[10px] font-semibold uppercase tracking-wider mb-0.5">Mensagem do Admin</p>

                                <p className="text-sm text-white leading-snug">{notif.mensagem}</p>
                          ...
                          <div className="p-6 text-center text-[#A0B3A6] text-xs flex flex-col items-center gap-2">

                            <Bell size={24} className="opacity-20 text-[#D4AF37]" />
